...
### Shipped
- Item list at `/inventory/item-master-data` with 17-column SAP-style table
- Item detail / edit at `/inventory/item-master-data/{id}/edit`
...
- 2026-04-28 — Item index aligned with the FL search data-table reference. Cell utility classes (`table-th` / `table-row` / `table-td`), `<x-data-table>` empty-state props, lead-column anchor unified to `text-blue-600`. Inline `text-[#2f5bff]` hex removed. Spec: `docs/superpowers/specs/2026-04-28-index-tables-fl-parity-design.md`.
- 2026-04-28 — Adopted FL Actions-column pattern. New `inventory.item-master-data.show` route (read-only by default with Edit Record toggle); existing `*.edit` route now pre-opens editing on first paint. Index gains an Actions column (was missing) with a `View | Edit` button pair; lead-column anchor flips to `*.show`. Form partial gains `$startEditing` flag and `match($mode)` for page title/description. Spec: `docs/superpowers/specs/2026-04-28-index-actions-view-edit-pair-design.md`.
- 2026-04-28 — Route family renamed from `system.item-master-data.*` at `/system/item-master-data...` to `inventory.item-master-data.*` at `/inventory/item-master-data...`. Old URLs and route names were removed to keep the module boundary clean. Spec: `docs/superpowers/specs/2026-04-28-master-data-route-module-rename-design.md`.
