...
- 2026-04-28 - Functional Location search (`/fleet/functional-location`) no longer shows the advanced filter modal. The page has been returned to a table-only browse screen with the shared datatable shell and existing `View | Edit` actions; the previously prototyped filter UI is removed from the page. 
- 2026-04-28 - Functional Location search (`/fleet/functional-location`) filter modal follow-up now mirrors the SAP tab layouts more literally and fixes the tall-modal viewport issue. The shared modal shell is now viewport-bounded with an internal scroll region, while the filter tabs were restructured to follow the reference screenshots for Functional Location, Properties, Part Information, Customers Information, and Flight OPS more closely. Mock `Apply` / `Reset` / `Cancel` behavior remains unchanged. Spec: `docs/superpowers/specs/2026-04-28-functional-location-filter-sap-layout-followup-design.md`.
- 2026-04-28 - Functional Location search (`/fleet/functional-location`) filter modal was redesigned for a more compact ATP Enterprise presentation. The modal now removes the mock-only `Result` tab, uses a tighter five-tab set, reduces empty space, and groups customer/flight sections more efficiently while preserving the existing `Filter` entry point, modal workflow, and mock `Apply` / `Reset` / `Cancel` behavior. Spec: `docs/superpowers/specs/2026-04-28-functional-location-filter-redesign-design.md`.
- 2026-04-28 - Functional Location search (`/fleet/functional-location`) now includes a modal-based advanced filter preview. A `Filter` trigger opens a tabbed SAP-structured filter workspace rendered with the current ATP Enterprise UI layer, with mock `Apply` / `Reset` / `Cancel` behavior and the existing result table preserved on the page. No real filtering logic yet; the first cut updates page-local preview/status state only. Spec: `docs/superpowers/specs/2026-04-28-functional-location-filter-modal-design.md`.
- 2026-04-28 - Equipment index (`/fleet/equipment`) follow-up aligned the visible table content to a user-provided SAP sample. The page now shows only the first three screenshot rows and removes the visible `Op. Code` / `Ow. Code` columns, while keeping the Equipment-specific chevron-circle `ID` cell and the ATP `Actions` column. Kept page-local by narrowing `EquipmentIndexPage` rather than changing shared Equipment data used elsewhere. Spec: `docs/superpowers/specs/2026-04-28-equipment-index-sap-content-followup-design.md`.
