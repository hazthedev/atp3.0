...
@section('title', 'Components Removed')
...
@section('content')
    <div
        ...
        x-data="{
            ...
            confirmSelection() {
                this.statusMessage = `Components removed workspace confirmed for ${this.filteredRows().length} record(s).`;
            },
        }"
    >
        <x-page-header
            title="Components Removed"
            description="Warehouse intake workspace for reviewing removed components, filtering by removal date, and preparing receipt or purchasing actions in the current ATP design system."
        />
        ...
        <section class="max-w-[1280px] space-y-5">
            ...
            <div class="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                <h3 class="mb-4 text-sm font-semibold text-gray-900">List of Removed Components</h3>

                <div class="grid items-end gap-4 xl:grid-cols-[220px_200px_auto]">
                    <div>
                        <label for="component_removed_from" class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-gray-500">From</label>
                        <x-date-picker id="component_removed_from" x-model="fromDate" />
                    </div>

                    <div>
                        <label for="component_removed_to" class="mb-1.5 block text-xs font-semibold uppercase tracking-wider text-gray-500">To</label>
                        <x-date-picker id="component_removed_to" x-model="toDate" />
                    </div>

                    <div>
                        <button type="button" class="btn-secondary" @click="searchRows()">Search</button>
                    </div>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-gray-200 bg-white shadow-sm">
                <div class="overflow-x-auto">
                    <table class="min-w-full border-collapse min-w-[2200px]">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3 border-t border-gray-200 pt-5">
                ...
                <div class="flex flex-wrap items-center gap-3">
                    <button type="button" class="btn-secondary" @click="createGoodsReceipt()">Create Goods Receipt</button>
                    <button type="button" class="btn-secondary" @click="createPurchaseRequest()">Create Purchase Request</button>
                    <button type="button" class="btn-secondary" @click="createPurchaseOrder()">Create Purchase Order</button>
                </div>
            </div>
        </section>
    </div>
