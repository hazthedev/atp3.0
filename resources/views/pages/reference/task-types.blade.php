...
@section('content')
    @php
        ...
        $rows = $types->map(function ($t) use ($primary) {
            $badge = in_array($t->name, $primary, true)
                ? '<span class="ml-2 inline-flex rounded-full bg-blue-100 px-2 py-0.5 text-[10px] font-semibold text-blue-700">Fleet Synthesis primary</span>'
                : '';
            return [
    ...
    @include('pages.partials.reference-table-page', [
        'title' => 'Task Types',
        'description' => 'Task type catalogue from SAP @MRO_OTAT. Seven of these are the primary Fleet Synthesis Dashboard columns (AD/SB, EOAS, OOP, Kardex, Others, Checks, MEL/CFD).',
        'columns' => ['Code', 'Name'],
        'rows' => $rows,
        'source' => '@MRO_OTAT',
    ])
