@php
    ...
    $isL1 = $level === 0;
    // Only L1 items with children dispatch "expand all descendants" on click.
    $expandAllExpression = $isL1 && $hasChildren
        ? 'if (open) { $el.closest(\'li\').querySelectorAll(\'[data-sidebar-node]\').forEach(n => n.dispatchEvent(new CustomEvent(\'sidebar-expand\'))); }'
        : '';
@endphp

<li class="space-y-1" x-data="{ open: @js($active) }" data-sidebar-node @sidebar-expand="open = true">
    @if ($hasChildren && $hasRoute)
        <div class="{{ $linkClass }} justify-between" style="padding-left: {{ $indent }}rem;">
            <a href="{{ route($item['route']) }}" class="flex min-w-0 flex-1 items-center gap-3" title="{{ $item['label'] }}" @click="open = true; {{ $expandAllExpression }}">
                <x-icon :name="$item['icon'] ?? 'document'" class="h-5 w-5 shrink-0" />
                <span x-cloak x-show="!sidebarCollapsed" class="truncate">{{ $item['label'] }}</span>
            </a>
            <button type="button" class="{{ $chevronButtonClass }}" @click.stop="open = !open; {{ $expandAllExpression }}" x-cloak x-show="!sidebarCollapsed" aria-label="Toggle section">
                <x-icon name="chevron-down" class="{{ $chevronIconClass }}" x-bind:class="open ? 'rotate-180' : ''" />
            </button>
        </div>
    @elseif ($hasChildren)
        <div class="{{ $linkClass }} justify-between" style="padding-left: {{ $indent }}rem;">
            <button type="button" class="flex min-w-0 flex-1 items-center gap-3 text-left" @click="open = !open; {{ $expandAllExpression }}" title="{{ $item['label'] }}">
                <x-icon :name="$item['icon'] ?? 'document'" class="h-5 w-5 shrink-0" />
                <span x-cloak x-show="!sidebarCollapsed" class="truncate">{{ $item['label'] }}</span>
            </button>
            <button type="button" class="{{ $chevronButtonClass }}" @click.stop="open = !open; {{ $expandAllExpression }}" x-cloak x-show="!sidebarCollapsed" aria-label="Toggle section">
