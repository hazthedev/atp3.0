    ...
    <x-card title="Swap Equipment" description="Choose the equipment to swap and review its father equipment context." padding="p-6">
        <div class="space-y-8">
                    ...
                    <div class="space-y-3">
                        ...
                        <div class="grid gap-4 md:grid-cols-2">
                            @foreach ($equipmentFields as $field)
                                <x-form.input
                                    :label="$field['label']"
                                    :name="'swap_' . $field['key']"
                                    :value="$equipmentToSwap[$field['key']] ?? ''"
                                    readonly
                                    :class="filled($equipmentToSwap[$field['key']] ?? '') ? 'input-field-filled' : ''"
                                />
                            @endforeach
                        </div>
                    </div>
            ...
            <div class="mt-5 flex flex-wrap items-center justify-end gap-3">
                ...
                <button type="button" class="btn-secondary" wire:click="swapPreview" @disabled($equipmentId === null)>Swap</button>
            </div>
        </div>
    </x-card>
                    ...
                    <div class="relative flex max-h-[65vh] flex-col overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl">
                        <div class="flex items-start justify-between gap-4 border-b border-gray-200 px-5 py-4">
                            <div>
                                ...
                                <p class="mt-1 text-sm text-gray-500">Search Equipments modal with choose/cancel actions for the swap workflow.</p>
                            </div>
                                    ...
                                    <x-slot name="tbody">
                                        @foreach ($searchResults as $row)
                                            <tr
                                                ...
                                                wire:key="swap-equipment-search-{{ $row['id'] }}"
