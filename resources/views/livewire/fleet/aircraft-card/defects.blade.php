...
<div class="space-y-4">
    ...
    <div class="flex flex-wrap items-center gap-2 rounded-xl border border-gray-200 bg-white p-3 shadow-sm">
        <input type="text" placeholder="Search by description, A.A. No, employee, part number..." class="input-field w-72 text-xs" />
        <select class="input-field w-32 text-xs"><option>All Type</option></select>
        <select class="input-field w-32 text-xs"><option>All MEL Status</option></select>
        <select class="input-field w-36 text-xs"><option>All Defect Category</option></select>
        ...
        <button type="button" class="ml-auto inline-flex items-center gap-1.5 rounded-md border border-gray-300 bg-white px-3 py-1.5 text-xs font-medium text-gray-700 hover:bg-gray-50">
            <x-icon name="cog-6-tooth" class="h-3.5 w-3.5" /> Manage Defects
        </button>
    </div>

    <section class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
        <h3 class="mb-3 text-sm font-semibold text-gray-900">Defects ({{ count($d['rows']) }})</h3>
        <div class="overflow-x-auto">
            <table class="w-full text-[11px]">
                <thead>
                    <tr class="border-b border-gray-200 bg-gray-50 text-left uppercase tracking-wide text-gray-500">
                        <th class="py-1.5 pl-2" rowspan="2">Type</th>
                        <th class="py-1.5" rowspan="2">Date</th>
                        <th class="py-1.5" rowspan="2">Time</th>
                        <th class="py-1.5" rowspan="2">Aircraft Hours (FH)</th>
                        <th class="py-1.5" rowspan="2">Aircraft Cycles (FC)</th>
                        <th class="py-1.5" rowspan="2">A.A. No</th>
                        <th class="py-1.5" rowspan="2">Description</th>
                        <th class="py-1.5 text-center" colspan="2">Reported By</th>
                        <th class="py-1.5" rowspan="2">Action Taken</th>
                        <th class="py-1.5 text-center" colspan="2">Performed By</th>
                        <th class="py-1.5" rowspan="2">Deferral</th>
                        <th class="py-1.5" rowspan="2">MEL Reference No.</th>
                        <th class="py-1.5" rowspan="2">MEL Repair Category</th>
                        <th class="py-1.5" rowspan="2">MEL Expiry Date</th>
                        <th class="py-1.5" rowspan="2">Reason for Deferral</th>
                        <th class="py-1.5 text-center" colspan="4">Part Number with Quantity, Description and Remarks</th>
                        <th class="py-1.5" rowspan="2">Defect Category</th>
                    </tr>
                    ...
                    @foreach ($d['rows'] as $r)
                        <tr>
                            <td class="py-1.5 pl-2">
                                <span class="inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[10px] font-semibold {{ $r['tone'] === 'red' ? 'bg-red-100 text-red-800' : ($r['tone'] === 'amber' ? 'bg-amber-100 text-amber-800' : 'bg-gray-100 text-gray-700') }}">
                            ...
                            <td class="py-1.5">{{ $r['aircraft_hours'] }}</td>
                            <td class="py-1.5">{{ $r['aircraft_cycles'] }}</td>
                            <td class="py-1.5">{{ $r['aa_no'] }}</td>
    ...
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
        <section class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <h4 class="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">Type Legend</h4>
            <ul class="space-y-1.5 text-[11px] text-gray-700">
                <li class="flex items-center gap-2"><span class="inline-flex items-center gap-1 rounded bg-red-100 px-1.5 py-0.5 text-[10px] font-semibold text-red-800"><x-icon name="exclamation-triangle" class="h-3 w-3" /> MAREP</span> Maintenance Report</li>
                <li class="flex items-center gap-2"><span class="inline-flex items-center gap-1 rounded bg-amber-100 px-1.5 py-0.5 text-[10px] font-semibold text-amber-800"><x-icon name="bell" class="h-3 w-3" /> PIREP</span> Pilot Report</li>
                <li class="flex items-center gap-2"><span class="inline-flex items-center gap-1 rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-semibold text-gray-700"><x-icon name="information-circle" class="h-3 w-3" /> INFO</span> Informational Report</li>
            </ul>
        </section>
        <section class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <h4 class="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">MEL Repair Category Legend</h4>
            <ul class="space-y-1 text-[11px] text-gray-700">
                ...
                <li><span class="rounded bg-gray-100 px-1.5 py-0.5 text-[10px] font-semibold text-gray-700">D</span> No Safety Effect (Operational)</li>
        ...
        <section class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
            <h4 class="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500">Defect Category Legend</h4>
            <ul class="space-y-1 text-[11px] text-gray-700">
